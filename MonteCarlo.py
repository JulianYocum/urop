...
class MonteCarlo(DataAnalysis):
    def __init__(self, num_tracks=None, track_type=None, particle_type=None, allow_empty_tracks=False, flux_square_size=12):
        ...
        for i in range(len(self.tracks)):
            ...
            if len(channels) > 0:
                for j in range(len(channels)):
                    row = [0, int(channels[j]), rand_times[i] + j*.01, 1, 3.2, 3.2, dEdxs[j] * distances[j], 0.0, 0.0, 0, False, i, 0, i, True, distances[j]]
                    events.append(row)
