...
void B2aDetectorConstruction::DefineMaterials()
{
  ...
  a = 127.6*g/mole;
  G4Element* elTe  = new G4Element(name="Te",symbol="H" , z= 52., a);

  a = 16.00*g/mole;
  G4Element* elO  = new G4Element(name="O"  ,symbol="O" , z= 8., a);
...
G4VPhysicalVolume* B2aDetectorConstruction::DefineVolumes()
{
  ...
  //G4double trackerLength = 1.0*cm;

  G4double worldLength = targetLength * sqrt(3);
  ...
  G4Sphere* worldS = new G4Sphere("world", 0, worldLength/2, 0, 4*acos(0.0), 0, 2*acos(0.0));
  /*
  ...
  */
  ...
  G4Box* targetS = new G4Box("target",targetLength/2,targetLength/2,targetLength/2);
  //    = new G4Tubs("target",0.,targetRadius,targetLength,0.*deg,360.*deg);

  fLogicTarget = new G4LogicalVolume(targetS, fTargetMaterial,"Target_LV",0,0,0);
...
/*
  ...
  G4double maxStep = 0.1*targetLength;
...
void B2aDetectorConstruction::ConstructSDandField()
{
  ...
  // of "Chamber_LV".
  SetSensitiveDetector("Target_LV", aTrackerSD, true);
