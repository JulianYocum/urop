                                     ...
                                     G4TouchableHistory*)
{
  ...
  kenergy = aStep->GetTrack()->GetDynamicParticle()->GetKineticEnergy();
  mvector = aStep->GetTrack()->GetDynamicParticle()->GetMomentumDirection();
  G4String name = aStep->GetTrack()->GetParticleDefinition()->GetParticleName();
  //std::cout << edep << std::endl;
  //std::cout << kenergy << std::endl;
  //std::cout << mvector << std::endl;

  //std::cout << "Get Particle Name: " << aStep->GetTrack()->GetParticleDefinition()->GetParticleName() << std::endl;
  ...
  newHit->SetKE(kenergy);
  newHit->SetMvec(mvector);
...
void B2TrackerSD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  /*
  std::ofstream f_denergy, f_kenergy;

  f_denergy.open("denergies.txt", std::ios_base::app);
  f_kenergy.open("kenergies.txt" , std::ios_base::app);

  f_denergy <<  denergy << std::endl;
  f_kenergy << kenergy  << std::endl;
